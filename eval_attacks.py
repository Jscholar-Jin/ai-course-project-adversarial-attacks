# eval_attack_effect.py
# 功能：
# 检测对抗攻击效果
#
# 输出：
# 1. results/attack_effect_results.csv
# 2. figures/attack_adv_acc_bar.png
# 3. figures/attack_asr_bar.png
#
# 指标：
# Clean Acc：干净样本准确率
# Adv Acc：对抗样本准确率
...
def main():
    parser = argparse.ArgumentParser(
        description="检测 FGSM / PGD / DeepFool / MI-FGSM / SPSA 攻击效果"
    )
    ...
    # 黑盒攻击：SPSA Query Model B -> Model B
    experiments.append(
        {
            "experiment": "black-box",
            "attack_name": "spsa",
            "method_label": "Black-box SPSA",
            "source_model_name": "Query Model B",
            "target_model_name": "Model B",
            "source_model": model_b,
            "target_model": model_b,
    ...
    csv_path = os.path.join(args.result_dir, "attack_effect_results.csv")
    ...
    adv_acc_fig = os.path.join(args.figure_dir, "attack_adv_acc_bar.png")
    asr_fig = os.path.join(args.figure_dir, "attack_asr_bar.png")

    plot_bar(
        df=df,
        value_col="adv_acc",
        title="Adversarial Accuracy under Different Attacks",
        ylabel="Adv Acc (%)",
    ...
    plot_bar(
        df=df,
        value_col="asr",
        title="Attack Success Rate under Different Attacks",
        ylabel="ASR (%)",
        save_path=asr_fig,
    )
